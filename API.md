...
The following methods are available for `CraftTool` objects:
 ...
 - `CraftTool 'mymod:item': require_uses(uses)`: The recipe will not be craftable if the item has less than this number of uses remaining, unless `any_uses` was specified. Does not overwrite the minimums set by other overrides, e.g. `take_uses`. When the value is -1, requires a fully intact item without need to specify its' max uses directly.
